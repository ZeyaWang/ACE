...
def build_depict(input_var=None, n_in=[None, None, None], feature_map_sizes=[50, 50],
    ...
    return l_e3, l_d0, loss_recons, loss_recons_clean, l_e2_flat, l_d2_flat
...
def train_depict(dataset, X, y, input_var, decoder, encoder, encoder1, encoder2, loss_recons, num_clusters, y_pred, output_path,
    ...
    feature_prediction1 = lasagne.layers.get_output(encoder1, input_var, deterministic=True)
    feature_prediction2 = lasagne.layers.get_output(encoder2, input_var, deterministic=True)
    ...
    feature_prediction_fn1 = theano.function([input_var], feature_prediction1)
    feature_prediction_fn2 = theano.function([input_var], feature_prediction2)
    ...
    for batch in iterate_minibatches(X, y, test_batch_size, shuffle=False):
        ...
        y_features1.append(np.array(feature_prediction_fn1(minibatch_inputs)))
        y_features2.append(np.array(feature_prediction_fn2(minibatch_inputs)))
        y_pred[idx] = minibatch_pred

    y_features = np.concatenate(y_features, axis=0)
    y_features1 = np.concatenate(y_features1, axis=0)
    y_features2 = np.concatenate(y_features2, axis=0)

    np.savez('output{}_{}.npz'.format(dataset, num_clusters), 
        y_features=y_features, y_pred=y_pred, y_features1=y_features1, y_features2=y_features2)
