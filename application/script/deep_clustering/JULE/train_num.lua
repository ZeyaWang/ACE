...
cmd:option('-numcl', 10, ' the number of clusters')
...
for n = 1, opt.epoch_rnn do
  ...
  while 1 do
    ...
    if is_allfinished() then
      ...
      --gtlabelname = "gtlabel"..opt.dataset..opt.numcl..".h5"
      --local gtlabelFile = hdf5.open(gtlabelname, 'w')
      --gtlabelFile:write('label', label_gt_table_table[1])
      --gtlabelFile:close()
      
      --prelabelname = "prelabel"..opt.dataset..opt.numcl..".h5"
      --local prelabelFile = hdf5.open(prelabelname, 'w')
      --prelabelFile:write('label', label_pre_table_table[1])
      --prelabelFile:close()
